...
function SettingsView({ sessions, onImported, notify, dark, onToggleTheme }: { sessions: Session[]; onImported: (sessions: Session[]) => void; notify: (toast: Toast) => void; dark: boolean; onToggleTheme: () => void }) {
  ...
  const exportJson = () => { downloadFile(toJson(sessions), `armx-${new Date().toISOString().slice(0, 10)}.json`, "application/json"); notify({ message: "Données JSON exportées." }); };
  const exportCsv = () => { downloadFile(toCsv(sessions), `armx-${new Date().toISOString().slice(0, 10)}.csv`, "text/csv;charset=utf-8"); notify({ message: "Données CSV exportées." }); };
